      ...
      <el-col :span="24">  
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.dianputupian" label="店铺图片" prop="dianputupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.dianputupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="主营业务" prop="zhuyingyewu">
          <el-input v-model="ruleForm.zhuyingyewu" 
              placeholder="主营业务" clearable  :readonly="ro.zhuyingyewu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="主营业务" prop="zhuyingyewu">
              <el-input v-model="ruleForm.zhuyingyewu" 
                placeholder="主营业务" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="店铺简介" prop="dianpujianjie">
                <el-input
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.dianpujianjie" label="店铺简介" prop="dianpujianjie">
                    <span>{{ruleForm.dianpujianjie}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      this.$http({
		...
		if(json.dianzhangxingming!=''&&json.dianzhangxingming){
                this.ruleForm.dianzhangxingming = json.dianzhangxingming
	    		this.ro.dianzhangxingming = true;
		}
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `dayindian/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.dayindianCrossAddOrUpdateFlag = false;
						 this.parent.search();
